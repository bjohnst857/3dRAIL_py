"""IMEX time steps for the 3D RAIL viscous Burgers solver, orders 1-3.
...
tableau; the nonlinear advection flux is treated *explicitly*; the known source
``phi`` is evaluated at the implicit stage nodes.  Everything stays in Tucker
format.
...
The only difference from the sibling ``linear_advection_diffusion`` port is the
flux term itself: linear advection has three *independent* linear fluxes
E1=a1*u, E2=a2*u, E3=a3*u built from external flow fields A(t),B(t),C(t).
Burgers' flux is a *self* Tucker-Hadamard product u^2/2 -- the SAME tensor
E1=E2=E3, computed once per required time level and reused for all three
directional derivatives (see :func:`burgers_flux`/:func:`burgers_flux_terms`).
Every other piece of machinery below -- the K-step/S-step Sylvester solves,
reduced augmentation, and the exact DIRK/RK coefficients -- is identical to
the linear-advection-diffusion port, since MATLAB's helper files
(TKR.m, tkron.m, red_aug_K.m, red_aug_S.m, simoncini_direct_solver.m,
nonconstrun.m) are byte-identical between the two PDEs. The ``A``, ``B``, ``C``
parameters threaded through every function below are dead (never called) --
kept only for call-signature parity with the linear-advection-diffusion port
and with ``test_parameters.py`` (which already sets them to ``lambda t: 0``),
exactly mirroring how MATLAB's ``main.m``/``IMEX*.m`` keep them as unused args.
...
def burgers_flux(sol):
    """The nonlinear Burgers flux u^2/2 as a Tucker tensor.

    The self Tucker-Hadamard product of the solution with itself.  Mirrors
    MATLAB's ``E_n = {{TKR(U_n{1},U_n{1}),TKR(U_n{2},U_n{2}),TKR(U_n{3},U_n{3})},
    tkron(0.5*G_n,G_n)}``, where E1=E2=E3 are identical (Burgers has a single
    scalar flux function f(u)=u^2/2 differentiated in each of x,y,z, unlike
    linear advection's three independent flux fields).  Exact -- no Newton or
    fixed-point iteration -- but squares the per-mode Tucker rank before
    truncation.

    Parameters
    ----------
    sol : (factors, core)   Tucker tensor for u

    Returns
    -------
    (factors, core) : Tucker tensor for u^2/2
    """
    factors, core = sol
...
#                                        pieces, one per mode)
#                        'flux'      -> one advection flux d/dx_d (u^2/2):
...
def burgers_flux_terms(coeff, sol):
    """The three flux terms d/dx(u^2/2), d/dy(u^2/2), d/dz(u^2/2) for one ``sol``.

    ``coeff`` already includes the (signed) dt weight.  Unlike linear
    advection's flux terms, all three directions reuse the SAME flux tensor
    (Burgers' flux is a self-product, not three independent flow fields times
    u) -- computed once here and shared, never recomputed per direction.
    """
    E = burgers_flux(sol)
    return [
        (coeff, "flux", E, 0),   # d/dx (u^2/2)
        (coeff, "flux", E, 1),   # d/dy (u^2/2)
        (coeff, "flux", E, 2),   # d/dz (u^2/2)
    ]
...
def imex111(U_n, G_n, MLR_n, A, B, C, P, tn, dtn, diff, tol, lomac_data=None):
    ...
    right-hand side: transport of u^n, the Burgers flux at t^n (explicit,
    ...
    A,B,C : callable(t)       unused; kept for call-signature parity with the
                               linear-advection-diffusion port (Burgers has no
                               independent flow fields -- see module docstring)
    ...
    terms += burgers_flux_terms(-dtn, sol_n)
...
def imex222(U_n, G_n, MLR_n, A, B, C, P, tn, dtn, diff, tol, lomac_data=None):
    ...
    terms += burgers_flux_terms(-delta*dtn,     sol_n)  # flux at t^n
    terms += burgers_flux_terms(-(1-delta)*dtn, sol_1)  # flux at t^(1)
...
def imex443(U_n, G_n, MLR_n, A, B, C, P, tn, dtn, diff, tol, lomac_data=None):
    ...
    terms += burgers_flux_terms(-(11/18)*dtn, sol_n)   # E_n at t^n
    terms += burgers_flux_terms(-(1/18)*dtn,  sol_1)   # E_1 at t^(1)
    ...
    terms += burgers_flux_terms(-(5/6)*dtn, sol_n)
    terms += burgers_flux_terms(+(5/6)*dtn, sol_1)     # -(-5/6)
    terms += burgers_flux_terms(-(1/2)*dtn, sol_2)
    ...
    terms += burgers_flux_terms(-(1/4)*dtn, sol_n)
    terms += burgers_flux_terms(-(7/4)*dtn, sol_1)
    terms += burgers_flux_terms(-(3/4)*dtn, sol_2)
    terms += burgers_flux_terms(+(7/4)*dtn, sol_3)     # -(-7/4)
