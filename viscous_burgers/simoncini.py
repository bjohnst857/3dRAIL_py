...
def simoncini(A1, A2, A3, M1, M2, H1, H3, B, check_res=False):
    ...
    # Every iteration solves a Sylvester equation  P*X + X*Q_syl = F  whose
    # P = M2\A1 is the SAME for all k -- only Q_syl (via the scalar R[k,k]) and F
    # change.  The Bartels-Stewart algorithm Schur-decomposes both P and Q_syl;
    # we Schur-decompose P once here and reuse it across the loop, leaving only
    # the small per-iteration Schur of Q_syl.  This mirrors the K-step caching
    # and is exactly what makes the inner loop (length R, up to ~25) cheap.
    P = M2_inv_A1
    Rp, Up = schur(P, output='real')       # P = Up @ Rp @ Up.T  (computed once)
    UpH = Up.conj().T
    trsyl, = get_lapack_funcs(('trsyl',), (Rp,))
    ...
    for k in range(ng):
        ...
        # Below is solve_sylvester(P, Q_syl, F) with the Schur of P reused.
        Q_syl  = (R[k, k] * H3.T + A2.T) @ M1_inv_T
        Rq, Vq = schur(Q_syl.conj().T, output='real')
        f      = UpH @ F @ Vq
        y, scale, info = trsyl(Rp, Rq, f, tranb='C')
        Zhat   = Up @ (scale * y) @ Vq.conj().T
