"""3D RAIL IMEX viscous Burgers solver (Tucker format), orders 1/2/3.

Solves, with periodic boundary conditions on [-L/2, L/2]^3,

    u_t + (u^2/2)_x + (u^2/2)_y + (u^2/2)_z = d1 u_xx + d2 u_yy + d3 u_zz + phi.

Diffusion is treated implicitly and the nonlinear advection flux + source
explicitly.  The time integrator is selectable: first-order IMEX111,
second-order IMEX222, or third-order IMEX443 (see imex.py).  The solution
stays in Tucker low-rank format.

Command-line usage
------------------
    python main.py [--order {1,2,3}] [--test {1,2}] [--sweep {0,1}]

--order  IMEX order of accuracy: 1=IMEX111, 2=IMEX222, 3=IMEX443.  Default: 1.
--test   Which test case to run (1 or 2; see test_parameters.py).  Default: 1.
             1 -> accuracy test, rank-(2,2,2) IC, manufactured exact solution
             2 -> rank-growth test, no source, no exact solution
...
# MATLAB's main.m aborts interactively (input()) when the multilinear rank
# exceeds this threshold; in batch mode we just log a warning and continue
# (see run_lad_test.m's non-interactive precedent for the sibling solver,
# whose threshold is 25 -- viscous Burgers' self-product flux inflates rank
# faster, hence the higher threshold here).
RANK_WARN_THRESHOLD = 37

# MATLAB's viscous-Burgers main.m uses a Gaussian LoMaC weight decay of s=5
# (vs. s=1 for the sibling linear-advection-diffusion solver).
LOMAC_S = 5.0
...
def first_derivative_matrix(N, ds, L):
    ...
    Skew-symmetric Toeplitz matrix (cot-based).  Used for the advection flux.
    """
    k = np.arange(1, N)
    col = np.zeros(N)
...
def run(testnumber, lam, Tf, N, tol, order, lomac_level=None):
    ...
    lomac_data = build_lomac_data(
        p, U, G, level=lomac_level if lomac_level is not None else 2, s=LOMAC_S)
    ...
    for n in range(1, Nt):
        ...
        U, G, MLR = imex_step(order, U, G, MLR, p.A, p.B, p.C, p.P, tn, dtn,
                              diff, tol, truncation)
        rankvals[n, :] = MLR
        if max(MLR) > RANK_WARN_THRESHOLD:
            print(f"Multilinear rank is {MLR}; continuing (batch mode).")
...
def main():
    parser = argparse.ArgumentParser(
        description="3D RAIL IMEX viscous Burgers solver (orders 1, 2, 3).",
    ...
    parser.add_argument("--test", type=int, choices=[1, 2], default=1,
                        help="test case (1 or 2; see test_parameters.py). Default: 1.")
    parser.add_argument("--sweep", type=int, choices=[0, 1], default=0,
                        help="0=single lambda; 1=lambda sweep. Default: 0.")
    parser.add_argument("--Tf", type=float, default=1, help="final time. Default: 1.")
    parser.add_argument("--N", type=int, default=150, help="cells per dimension. Default: 150.")
    parser.add_argument("--tol", type=float, default=1e-5, help="truncation tolerance. Default: 1e-5.")
