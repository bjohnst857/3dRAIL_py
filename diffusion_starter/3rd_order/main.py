"""3D RAIL third-order (DIRK3 / ARS(4,4,3)) diffusion solver (Tucker format).

Solves   u_t = d1*u_xx + d2*u_yy + d3*u_zz   on a periodic box, advancing the
low-rank Tucker solution with the four-stage DIRK3 step (``integrators.dirk3``),
the implicit half of the MATLAB IMEX443 scheme.
...
from integrators import DiffMatrices, make_diff_matrix, dirk3

# -------------------------------------------------------------------------
# Command-line arguments (all optional; see the module docstring above)
# -------------------------------------------------------------------------
parser = argparse.ArgumentParser(
    description="3D RAIL third-order (DIRK3) diffusion solver (Tucker format).",
...
for k in range(len(Lambdavals)):
    ...
    # ---------------------------------------------------------------------
    # Time loop (third-order DIRK3 step)
    # ---------------------------------------------------------------------
    for n in range(1, Nt):
        ...
        U, G, MLR = dirk3(U, G, MLR, diff, dtn, tol)
        rankvals[n, :] = MLR
...
# L1 error vs lambda, with an order-3 reference line
...
if i1 > i0:
    ax2.loglog(Lambdavals[i0:i1], 0.00005*Lambdavals[i0:i1]**3, 'k-.',
               linewidth=1.5, label='Order 3')
