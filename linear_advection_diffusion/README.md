...
This folder is a Python port of the MATLAB code in
`3dRAIL/rail3d_sourcecode/rail3d_linear_advection_diffusion/`.  It solves a 3D
linear advection-diffusion equation while keeping the solution in a **low-rank
Tucker format** the whole time, using the **RAIL** integrator (a "rank-adaptive
...
The MATLAB `main.m` and the `IMEX*.m` files also track conserved quantities
(mass, momentum, energy) and use a **conservative LoMaC truncation**
(`lomac_012`).  This Python port uses the simpler, non-conservative truncation
`nonconstrun` instead, and does not track the macroscopic quantities.  This
keeps the port focused on the core RAIL algorithm.  See "Notes / open
questions" below.
...
The eight test cases live in `test_parameters.py`; their docstrings describe the
PDE, initial condition, and (where available) exact solution for each.
...
- **LoMaC / conservation not ported.** The MATLAB integrators truncate with the
  conservative `lomac_012`, which preserves mass/momentum/energy; this port uses
  the plain `nonconstrun`.  For accuracy tests (tests 1, 2, 3, 6) the results
  match expected orders, but if you need exact mass/energy conservation
  (especially the Dougherty-Fokker-Planck test 8), the LoMaC truncation would
