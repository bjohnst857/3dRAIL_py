...
Note on the weight-decay parameter ``s``: MATLAB's viscous-Burgers ``main.m``
uses ``s=5`` (vs. ``s=1`` for the sibling linear-advection-diffusion solver),
so unlike that port -- which hardcodes ``s=1.0`` internally -- this copy takes
``s`` as a parameter of :func:`build_lomac_data`.
...
def build_lomac_data(p, U0, G0, level=2, s=1.0):
    ...
    ``s`` is the Gaussian weight-decay rate (MATLAB's viscous-Burgers ``main.m``
    uses ``s=5``; the sibling linear-advection-diffusion solver uses ``s=1``).
    """
    ...
    # Weight function w = w1*w2*w3, sufficiently decaying at the boundary.
