"""3D RAIL IMEX advection-diffusion-reaction solver (Tucker format), orders 1/2/3.
...
(``build_diff_matrices``), so it is treated implicitly alongside diffusion at
no extra cost and requires no other change to the K/G-step machinery.

Diffusion (and reaction) are treated implicitly and advection + source
...
Command-line usage
------------------
    python main.py [--order {1,2,3}] [--sweep {0,1}]

--order  IMEX order of accuracy: 1=IMEX111, 2=IMEX222, 3=IMEX443.  Default: 1.
--test   Test case (see test_parameters.py). Only test 1 is defined: constant-
         coefficient advection-diffusion-reaction with a rank-3 IC and beta=1.0
         hardcoded (TestParameters.beta).
...
def build_diff_matrices(p):
    ...
    beta = p.beta
    Dxx = d1 * second_derivative_matrix(p.Nx, p.dx, p.L) + (beta / 3.0) * np.eye(p.Nx)
    Dyy = d2 * second_derivative_matrix(p.Ny, p.dy, p.L) + (beta / 3.0) * np.eye(p.Ny)
    Dzz = d3 * second_derivative_matrix(p.Nz, p.dz, p.L) + (beta / 3.0) * np.eye(p.Nz)

    return DiffMatrices(first=[Dx, Dy, Dz], second=[Dxx, Dyy, Dzz])


def _rel_drift(q, beta=0.0, t_final=0.0):
    ...
    initial profile, integrate to zero).
    """
    target = q[0] * np.exp(beta * t_final)
    return abs(q[-1] - target) / abs(target) if abs(target) > 1e-14 else abs(q[-1] - target)
...
def run(testnumber, lam, Tf, N, tol, order, lomac_level=None):
    ...
    Returns (L1err, rankvals, tvals, p, U, G, macro):
        ...
        p        : TestParameters (p.beta is the reaction coefficient used)
    ...
    assert p.beta <= 0 or dt * p.beta < 1.0, \
...
def main():
    ...
    parser.add_argument("--test", type=int, choices=[1], default=1,
                        help="test case (see test_parameters.py). Only 1 is defined. Default: 1.")
    ...
    for k, lam in enumerate(Lambdavals):
        ...
        # With beta != 0 these scale as exp(beta*t) rather than staying constant.
        mass_drift = _rel_drift(macro["mass"], p.beta, tvals[-1])
        energy_drift = _rel_drift(macro["energy"], p.beta, tvals[-1])
    ...
    def drift_curve(ax, series, name):
        q0 = series[0]
        target = q0 * np.exp(p.beta * tvals)
        if abs(q0) > 1e-14:
            ax.semilogy(tvals, np.abs(series - target) / np.abs(target), "k-")
            ax.set_ylabel(f"relative {name}\n|q - q0*e^(beta t)|/|q0*e^(beta t)|")
        else:
            ax.semilogy(tvals, np.abs(series - target), "k-")
