"""IMEX time steps for the 3D RAIL advection-diffusion-reaction solver, orders 1-3.

PDE (periodic BCs):

    u_t + (a1 u)_x + (a2 u)_y + (a3 u)_z = d1 u_xx + d2 u_yy + d3 u_zz + beta*u + phi

Diffusion and the linear reaction term (+beta*u) are treated *implicitly* with
a diagonally-implicit Runge-Kutta (DIRK) tableau; the advection fluxes are
treated *explicitly*; the known source ``phi`` is evaluated at the implicit
stage nodes.  Everything stays in Tucker format.

The reaction term needs no code changes in this file at all: ``main.py``
folds it into the diffusion matrices as F_tilde = F + (beta/3)*I before
...
class DiffMatrices:
    ...
    second[m]  : (N_m, N_m)  second-derivative matrix already scaled by the
                             ...
                             and already shifted by (beta/3)*I for the linear
                             reaction term if beta != 0 (see build_diff_matrices
                             in main.py)
    """
    first: list   # [Dx, Dy, Dz]
...
def implicit_stage(U_n, terms, a_diag, dtn, diff, tol, predictor, previous_U,
    ...
    t_new      : float              the time this stage's solution lands at
                                    (tn + c_i*dtn for this stage's node c_i;
                                    NOT always tn+dtn -- caller must supply it).
                                    Used only to rescale LoMaC's reference
                                    moments for the linear reaction term.
    ...
    return truncate(V_new, S_nn, tol, lomac_data, t_new)
...
def imex111(U_n, G_n, MLR_n, A, B, C, P, tn, dtn, diff, tol, lomac_data=None):
    ...
    return truncate(V_new, S_nn, tol, lomac_data, tn + dtn)
...
def imex222(U_n, G_n, MLR_n, A, B, C, P, tn, dtn, diff, tol, lomac_data=None):
    ...
    return implicit_stage(U_n, terms, gamma, dtn, diff, tol, U_dag, [U_1], lomac_data, tn + dtn)
...
def imex443(U_n, G_n, MLR_n, A, B, C, P, tn, dtn, diff, tol, lomac_data=None):
    ...
    t2 = tn + c2*dtn
    U_2, G_2, _ = implicit_stage(U_n, terms, a, dtn, diff, tol, U_dag, [U_1], lomac_data, t2)
    sol_2 = (U_2, G_2)
    ...
    t3 = tn + c3*dtn
    U_3, G_3, _ = implicit_stage(U_n, terms, a, dtn, diff, tol, U_dag, [U_1, U_2], lomac_data, t3)
    sol_3 = (U_3, G_3)
    ...
    return implicit_stage(U_n, terms, a, dtn, diff, tol, U_dag, [U_1, U_2, U_3], lomac_data, tn + dtn)
