...
class LomacData:
    ...
    JzM: float
    kM: float
    # linear reaction coefficient (+beta*u on the RHS); 0.0 -> original behavior
    beta: float = 0.0
...
def build_lomac_data(p, U0, G0, level=2):
    ...
    ``rhoM, JxM, JyM, JzM, kM`` from the initial condition ``(U0, G0)``.  The
    reaction coefficient ``p.beta`` is carried onto the returned
    :class:`LomacData` so :func:`lomac` can rescale those reference moments by
    exp(beta*t) instead of holding them fixed at t=0.
...
def lomac(factors, core, tol, data, t=0.0):
    ...
    rescaled by ``exp(data.beta * t)`` for the linear reaction term (with
    ``data.beta == 0`` this is exactly the original fixed-at-t=0 reference).
    ...
    # Conserved macroscopic part fM (from the reference moments, rescaled to
    # the current time for the linear reaction term).
    scale = np.exp(data.beta * t)
    S_fM = _macro_core(data.rhoM * scale, data.JxM * scale, data.JyM * scale,
                        data.JzM * scale, data.kM * scale, data)
...
def truncate(factors, core, tol, lomac_data=None, t=0.0):
    ...
    the truncation line in the MATLAB ``IMEX*.m`` files.  ``t`` is the time this
    tensor lands at (used only by :func:`lomac` to rescale reference moments
    for the linear reaction term).
    """
    if lomac_data is None:
        return nonconstrun(factors, core, tol)
    return lomac(factors, core, tol, lomac_data, t)
